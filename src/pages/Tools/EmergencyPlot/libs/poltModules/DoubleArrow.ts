...
import PlotHelper from './PlotHelper';
...
class DoubleArrow {
  ...
  draw(points: any) {
    ...
    // 如果长度大于等于3并且最后两点不重合
      ...
      // };
      leftArrowPnts = this.calculateControlPoints(
        pnt1,
        this.midTailPoint,
        this.pnt4,
        this.plotHelper.LEFT_SIDE,
      );
      rightArrowPnts = this.calculateControlPoints(
        this.midTailPoint,
        pnt2,
        pnt3,
        this.plotHelper.RIGHT_SIDE,
      );
  ...
  /**
   * 根据三个控制点和方向计算全部控制点
   * @param {object} pnt1 控制点1
   * @param {object} pnt2 控制点2
   * @param {object} pnt3 控制点3
   * @param {string} side 方向
   * @return {array} 返回控制点
   */
  calculateControlPoints(pnt1: any, pnt2: any, pnt3: any, side: any) {
    // 计算pnt1和pnt2的中心点
    var midPnt = this.plotHelper.getMidPoint(pnt1, pnt2);
    // 计算中心点到pnt3的距离
    var len = this.plotHelper.distance(midPnt, pnt3);
    // 计算第三点
    var midPnt1 = this.plotHelper.getThirdPoint(
      pnt3,
      midPnt,
      0,
      len * 0.3,
    ...
    var midPnt2 = this.plotHelper.getThirdPoint(
      pnt3,
      midPnt,
      0,
      len * 0.5,
    ...
    var midPnt3 = this.plotHelper.getThirdPoint(
      pnt3,
      midPnt,
      0,
      len * 0.7,
    ...
    midPnt1 = this.plotHelper.getThirdPoint(
      midPnt,
      midPnt1,
      Math.PI * 1.5,
      len / 4,
    ...
    midPnt2 = this.plotHelper.getThirdPoint(
      midPnt,
      midPnt2,
      Math.PI * 1.5,
      len / 4,
    ...
    midPnt3 = this.plotHelper.getThirdPoint(
      midPnt,
      midPnt3,
      Math.PI * 1.5,
      len / 4,
    ...
    // 箭头上全部控制点
    var arrowPnts = this.plotHelper.getArrowHeadPoints(
      points,
    ...
    // 尾部宽度因子
    var tailWidthFactor =
      this.plotHelper.distance(pnt1, pnt2) /
      this.plotHelper.getBaseLength(points) /
      2;
    // 左侧因子
    ...
    // 箭身
    var bodyPnts = this.plotHelper.getArrowBodyPoints(
      points,
      neckLeftPoint,
      neckRightPoint,
      tailWidthFactor,
  ...
  calculateFouthPoint(linePnt1: any, linePnt2: any, point: any) {
    var symPnt;
    var distance1;
    var distance2;
    var mid;
    ...
    if (angle < Math.PI / 2) {
      ...
      mid = this.plotHelper.getThirdPoint(
        linePnt1,
        midPnt,
        Math.PI * 1.5,
        distance1,
      ...
      symPnt = this.plotHelper.getThirdPoint(
        midPnt,
        mid,
        Math.PI * 1.5,
        distance2,
