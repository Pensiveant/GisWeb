  ...
  [4720],
  {
    14720: (e, t, a) => {
      ...
      async function u(e, t, a, r = {}) {
        ...
        try {
          var l;
          const { results: i, edits: n } = await p(e, t, a, r),
            o = (e) => e.filter((e) => !e.error).map(d.d9),
            u = {
              edits: n,
              addedFeatures: o(i.addFeatureResults),
              updatedFeatures: o(i.updateFeatureResults),
              deletedFeatures: o(i.deleteFeatureResults),
              addedAttachments: o(i.addAttachmentResults),
              updatedAttachments: o(i.updateAttachmentResults),
              deletedAttachments: o(i.deleteAttachmentResults),
            };
          return (
            null != (l = i.editedFeatureResults) &&
              l.length &&
              (u.editedFeatures = i.editedFeatureResults),
            (u.addedFeatures.length ||
              u.updatedFeatures.length ||
              u.deletedFeatures.length ||
              u.addedAttachments.length ||
              u.updatedAttachments.length ||
              u.deletedAttachments.length) &&
              e.emit('edits', u),
            s(u),
            i
          );
      ...
      async function p(e, t, a, r) {
        ...
        const { edits: d, options: l } = await (async function (e, t, a) {
          const r =
              t && (t.addFeatures || t.updateFeatures || t.deleteFeatures),
            d =
              t &&
              (t.addAttachments || t.updateAttachments || t.deleteAttachments);
          if (!t || (!r && !d))
            throw new i.Z(
              `${e.type}-layer:missing-parameters`,
          ...
          if (
            !e.capabilities.editing.supportsRollbackOnFailure &&
            a &&
            a.rollbackOnFailureEnabled
          )
            throw new i.Z(
              `${e.type}-layer:invalid-parameter`,
          ...
          const l = { ...a };
          if (
            (null != l.rollbackOnFailureEnabled ||
              e.capabilities.editing.supportsRollbackOnFailure ||
              (l.rollbackOnFailureEnabled = !0),
            !1 === l.rollbackOnFailureEnabled &&
              'original-and-current-features' === l.returnServiceEditsOption)
          )
            throw new i.Z(
              `${e.type}-layer:invalid-parameter`,
          ...
          if (
            !e.capabilities.editing
              .supportsReturnServiceEditsInSourceSpatialReference &&
            l.returnServiceEditsInSourceSR
          )
            throw new i.Z(
              `${e.type}-layer:invalid-parameter`,
          ...
          if (
            ...
            throw new i.Z(
              `${e.type}-layer:invalid-parameter`,
              "'returnServiceEditsOption' is valid only when 'returnServiceEditsOption' is set to 'original-and-current-features'",
            );
          const o = { ...t };
          if (
            ((o.addFeatures =
              t && s.Z.isCollection(t.addFeatures)
                ? t.addFeatures.toArray()
                : o.addFeatures || []),
            (o.updateFeatures =
              t && s.Z.isCollection(t.updateFeatures)
                ? t.updateFeatures.toArray()
                : o.updateFeatures || []),
            (o.deleteFeatures =
              t && s.Z.isCollection(t.deleteFeatures)
                ? t.deleteFeatures.toArray()
                : o.deleteFeatures || []),
            o.addFeatures.length && !e.capabilities.operations.supportsAdd)
          )
            throw new i.Z(
              `${e.type}-layer:unsupported-operation`,
              'Layer does not support adding features.',
            );
          if (
            o.updateFeatures.length &&
            !e.capabilities.operations.supportsUpdate
          )
            throw new i.Z(
              `${e.type}-layer:unsupported-operation`,
          ...
          if (
            o.deleteFeatures.length &&
            !e.capabilities.operations.supportsDelete
          )
            throw new i.Z(
              `${e.type}-layer:unsupported-operation`,
              'Layer does not support deleting features.',
            );
          (o.addAttachments = o.addAttachments || []),
            (o.updateAttachments = o.updateAttachments || []),
            (o.deleteAttachments = o.deleteAttachments || []),
            (o.addFeatures = o.addFeatures.map(m)),
            (o.updateFeatures = o.updateFeatures.map(m));
          const u = a && a.globalIdUsed;
          return (
            o.addFeatures.forEach((t) =>
              (function (e, t, a) {
                c(e, t, a);
              })(t, e, u),
            ),
            o.updateFeatures.forEach((t) =>
              (function (e, t, a) {
                if (
                  (c(e, t, a),
                  'geometry' in e &&
                    (0, n.pC)(e.geometry) &&
                    !t.capabilities.editing.supportsGeometryUpdate)
                )
                  throw new i.Z(
            ...
            o.deleteFeatures.forEach((t) =>
              (function (e, t, a) {
                c(e, t, a);
              })(t, e, u),
            ),
            o.addAttachments.forEach((t) => h(t, e)),
            o.updateAttachments.forEach((t) => h(t, e)),
            { edits: await y(o), options: l }
          );
        })(e, a, r);
        return d.addFeatures.length ||
          d.updateFeatures.length ||
          d.deleteFeatures.length ||
          d.addAttachments.length ||
          d.updateAttachments.length ||
          d.deleteAttachments.length
          ? { edits: d, results: await t.applyEdits(d, l) }
          : {
              edits: d,
              results: {
                addFeatureResults: [],
      ...
      function c(e, t, a) {
        ...
        if ('geometry' in e && (0, n.pC)(e.geometry)) {
          if (e.geometry.hasZ && !1 === t.capabilities.data.supportsZ)
            throw new i.Z(
              `${t.type}-layer:z-unsupported`,
              'Layer does not support z values while feature has z values.',
            );
          if (e.geometry.hasM && !1 === t.capabilities.data.supportsM)
            throw new i.Z(
      ...
      async function y(e) {
        const t = e.addFeatures,
          ...
          d = a.length;
        return (
          s.slice(0, i).forEach((t, a) => (e.addFeatures[a].geometry = t)),
          s
            .slice(i, i + d)
            .forEach((t, a) => (e.updateFeatures[a].geometry = t)),
          e
        );
      }
      function m(e) {
